...
| 박연수 | 이번 DevOps 프로젝트를 통해 단순히 “배포를 해본 경험”을 넘어서,
서비스를 운영한다는 관점에서 개발을 바라보는 시야를 넓힐 수 있었다고 느꼈다. 이번 프로젝트를 계기로 DevOps는 단순한 도구의 조합이 아니라 개발–배포–운영 전반을 하나의 흐름으로 설계하는 사고 방식이라는 점을 명확히 인식하게 되었다. 앞으로는 컴퓨터 네트워크와 Kubernetes 내부 동작에 대한 학습을 병행하며, 더 안정적이고 예측 가능한 시스템을 설계할 수 있는 개발자로 성장하고자 한다. |
| 배창민 | ㅇㅇ |
